...
function theme_almondb_page_init(moodle_page $PAGE) {
    global $CFG;
    // Версия по времени изменения файла: браузеры подхватывают обновлённый
    // frontpage.css сразу после деплоя, без ручного сброса кэша.
    $cssver = @filemtime($CFG->dirroot . '/theme/almondb/style/frontpage.css') ?: 1;
    $PAGE->requires->css(new moodle_url('/theme/almondb/style/frontpage.css', ['v' => $cssver]));
